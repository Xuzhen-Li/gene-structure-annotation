...
**When ready for `RUN=1`:** tick [Done when](https://github.com/Xuzhen-Li/gene-structure-annotation/wiki/Start-tonight#done-when) **and** `python3 pipeline/print_qc_commands.py` exits 0 with **no** `[STOP]`. Otherwise **no** `RUN=1` / no cluster run. Need HPC/containers before BRAKER/Liftoff.
